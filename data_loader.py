...
def load_truthfulqa_dataset(data_path: str) -> List[Dict]:
    ...
    if os.path.isdir(data_path):
        ...
        if filepath is None:
            ...
            logger.warning(f"[TruthfulQA] Could not find TruthfulQA.csv in {data_path}")
            logger.warning(f"[TruthfulQA] Files in directory: {all_files[:10]}...")
            return []
    else:
        filepath = data_path
    ...
    try:
        ...
        logger.error(f"[TruthfulQA] Failed to load {filepath}: {e}")
        return []
    ...
    for idx, row in df.iterrows():
        try:
            ...
            if not sample['answer_best']:
                true_answers = sample['answer_true']
                if true_answers:

                    if ';' in true_answers:
                        split_answers = true_answers.split(';')
                    else:
                        split_answers = [true_answers]
                    if split_answers:
                        sample['answer_best'] = split_answers[0].strip()
            
            if sample['answer_true'] and sample['answer_false']:
                dataset.append(sample)
        except Exception as e:
            logger.debug(f"[TruthfulQA] Skipping row {idx}: {e}")
            continue
